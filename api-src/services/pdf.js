...
export async function generateReportPDF({ request, tenant, report, organization }) {
  ...
  doc.fontSize(8).fillColor('#999').text(
    `${organization.name} · Generado por InmoBuró · Este reporte tiene validez de 30 días a partir de su emisión.`,
...
export async function generateContractPDF({ contract, request, tenant, organization }) {
  ...
  const clauses = [
    ...
    ['OCTAVA — CESIÓN DE DERECHOS.', `InmoBuró gestiona la presente operación. El ARRENDADOR acepta los términos de protección contratados según plan: ${request.plan.toUpperCase()}.`]
  ];
  ...
  doc.fontSize(8).fillColor('#999').text(
    `Documento generado digitalmente por InmoBuró · ${new Date().toLocaleString('es-MX')}`,
